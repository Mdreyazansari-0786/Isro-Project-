...
@app.route("/service")
def service():
    mycon=mysql.connector.connect(host="localhost",user="root",password="",database="project1")
    cur=mycon.cursor()
...
def insertdata():
    ...
    try:
        if(request.method=="POST"):
            ...
            dt={"username":user,"email":email,"phone":phone,"password":pw_hash.decode()}
...
def login_auth():
    ...
    if(request.method=="POST"):
        ...
        # print(retValue[4])
        # print(pas)
        if(bcrypt.check_password_hash(retValue['data'][2], pas)==True and email==retValue['data'][3]):
